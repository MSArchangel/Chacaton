...
def info_yach(cell_num: int):
    ...
    status_time_day = data[cell_num - 1]['status_d'][data[cell_num - 1]['status']]
    status_percentage_day = round(
        (data[cell_num - 1]['status_d'][data[cell_num - 1]['status']] / sum(
            data[cell_num - 1][
                'status_d'])) * 100, 2)
    status_time_hour = data[cell_num - 1]['status_h'][data[cell_num - 1]['status']]
    ...
    return [cell_num, name, avg_load_per_day, avg_load_per_hour, int_per_day, int_per_hour,
            cell_status_pie, cell_status_table,
